  ...
  return (
    <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden flex flex-col h-full">
        ...
        {tags.length > 0 && (
          <div className="absolute top-2 right-2 space-y-1">
            {tags.map((tag, idx) => (
              <span
                key={idx}
                className="inline-block bg-purple-500 text-white text-xs px-2 py-1 rounded"
              >
                {tag}
              </span>
            ))}
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <p className="text-gray-600 text-sm mb-2">{editorial}</p>
        <p className="text-gray-500 text-xs mb-3 line-clamp-2">{description}</p>

        {/* Price */}
        <div className="mb-3">
          <p className="text-2xl font-bold text-purple-600">S/ {pricePEN.toFixed(2)}</p>
        </div>
